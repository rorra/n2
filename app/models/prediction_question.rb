class PredictionQuestion < ActiveRecord::Base
  ...
  scope :top, lambda { |*args| { :order => ["prediction_guesses_count desc, created_at desc"], :limit => (args.first || 10)} }
  scope :currently_open, lambda { |*args| { :conditions => ["status = 'open'" ] } }
  scope :approved, :conditions => { :is_approved => true }
  #todo add migration for timestamp closed_at
  scope :closed, lambda { |*args| { :order => ["status = 'closed', updated_at desc"], :limit => (args.first || 7)} }
